...
public class ProjectFragment extends android.app.Fragment {
                             ...
                             final Bundle savedInstanceState)
    {
        ...
        searchCancle = (ImageView) view.findViewById(R.id.cancle_fragment_alert_numOfCar);
        search=(LinearLayout) view.findViewById(R.id.lav_alert_search);
        filterlist=(ListView)view.findViewById(R.id.lv_alert_filterlist);
        Name =(EditText) view.findViewById(R.id.edtTxt_fragment_alert_numOfCar);
        drawerLayout=(DrawerLayout) view.findViewById(R.id.drawer_layout_project);
        swipeRefreshLayout=(SwipeRefreshLayout)view.findViewById(R.id.srl_alert_pulltorefresh);
        filter=(LinearLayout)view.findViewById(R.id.lav_alert_filter);
        filterListAdapter=new FilterListAdapter(getActivity(),filterSelection);
        filterReset=(TextView)view.findViewById(R.id.tv_fragment_alert_filter_reset);
        filterSure=(TextView)view.findViewById(R.id.tv_fragment_alert_filter_sure) ;

        //UserIOUtil.setEditTextInhibitSpace(carNo);

        alert_progressBar=(ProgressBar)view.findViewById(R.id.alert_progressBar);
        blank = (TextView)view.findViewById(R.id.alert_blank);
        ...
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener(){
            ...
            public void onRefresh(){
                searchMode=0;
                    Map<String,Object> cautionsInfo = new HttpHandle(getActivity()).getCautionByFilterWithPage("","","","",1,number);
...
*/
    ...
    public void onActivityCreated(Bundle savedInstanceStace){
        ...
        //加载中的初始页面
        listView=(ListView) getView().findViewById(R.id.lv_fragment_alert_alertList);
            ...
            public void onClick(View v) {
                if (HttpHandle.isNetworkAvailable(getActivity())) {
                    ...
                    if(name.equals("")&&rank.equals("")&&type.equals("")&&batch.equals("")){
                        searchMode=0;
                    }else if(!name.equals("")&&rank.equals("")&&type.equals("")&&batch.equals("")){
                        searchMode=1;
                    }else {
                        searchMode=2;
                    }

                    Map<String, Object> queryResult = new HttpHandle(getActivity()).getCautionByFilterWithPage(name,type,rank,batch, 1, number);
                    ...
                    if (totalItemCount > 0) {
                        ...
                        if (nextpage <= totalPage && loadfinish[0]) {
                            ...
                            if(HttpHandle.isNetworkAvailable(getActivity())){
                                switch (searchMode){
                                    case 0:
                                        data =new HttpHandle(getActivity()).getCautionByFilterWithPage("","","","",nextpage,number);
                                        cautions = (List<Map<String,String>>) data.get("cautions");break;
                                    case 1:
                                        data=new HttpHandle(getActivity()).getCautionByFilterWithPage(name,"","","",nextpage,number);
                                        cautions = (List<Map<String,String>>) data.get("cautions");break;
                                    case 2:
                                        data=new HttpHandle(getActivity()).getCautionByFilterWithPage(name,type,rank,batch,nextpage,number);
                                        cautions = (List<Map<String,String>>) data.get("cautions");break;
                                    /*case 3:
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case 100:
                    ...
                    listView.setVisibility(View.VISIBLE);
                    blank.setVisibility(View.GONE);
                    ProjectFragment.this.alertMsgs.addAll((List<Map<String, String>>) msg.obj);
                ...
                case 101:
                    //页面初始化加载列表数据完成修改UI
                    try {
                        projectListViewAdapter= new ProjectListViewAdapter(alertMsgs, getActivity());
                        listView.setAdapter(projectListViewAdapter);
                    }catch (NullPointerException e){
                        e.printStackTrace();
                    }
